  ...
  const displayedZone =
    ...
    (() => {
      const zone = demoHeatZones.find(
        (item) => item.areaId === selectedWard.areaId,
      );

      return {
        ...zone,
        wardName: selectedWard.name,
        riskLevel: selectedWard.riskLevel,
        riskScore: selectedWard.riskScore,
        heatIndex: selectedWard.heatIndex,
        thermalStress: selectedWard.thermalStress,
  ...
  return (
    <div className="heatguard-shell">
      <aside className="heatguard-sidebar">
        <div className="brand">
          <span className="brand-flame">♨</span> HEATGUARD
        </div>
        ...
        <Button className="emergency-button" variant="outline-danger">
      ...
      <main className="heatguard-main">
        <section className="primary-dashboard-grid">
          <section className="map-panel heat-panel">
            <div
              ...
              aria-label="Synthetic demo heat risk map, not official ward boundaries"
            >
              <MapContainer
              ...
              >
                ...
                {demoHeatZones.map((geometry) => {
                  ...
                  const zone = {
                    ...
                    heatIndex: ward.heatIndex ?? "Demo",
                    population: ward.population ?? "Demo",
                  };

                  const isSelected = selectedZone?.areaId === zone.areaId;
                  return (
                    <Polygon
                      key={zone.areaId}
                      positions={zone.coordinates}
                      pathOptions={{
                        color: riskZoneColors[zone.riskLevel],
                        fillColor: riskZoneColors[zone.riskLevel],
                        fillOpacity: isSelected ? 0.7 : 0.48,
                    ...
                    >
                      <Popup>
                        <strong>{zone.wardName}</strong>
                        <br />
                        <small>
                          Synthetic demo zone — not an official ward boundary
                        </small>
                        <br />
                        Risk level: {displayLevel(zone.riskLevel)}
                        <br />
                        Risk score: {zone.riskScore}
                        <br />
                        Heat index: {zone.heatIndex}
                        <br />
                        Thermal stress: {zone.thermalStress} / 100
                        <br />
              ...
              <div className="ward-overlay">
                <div className="ward-title">
                  <div>
                    ...
                    <strong>{displayedZone.wardName}</strong>
                  </div>
                  <RiskBadge level={displayedZone.riskLevel} />
                </div>
                <div>
                  <span>
                    Heat Index <em>demo</em>
                  </span>
                ...
                <div>
                  <span>
                    Population <em>demo</em>
                  </span>
                  <b>{displayedZone.population}</b>
                </div>
                <div>
                  <span>Risk Score</span>
                  <b className="risk-score">{displayedZone.riskScore} / 100</b>
                </div>
              </div>
              <span className="map-demo-label">
                Synthetic/demo zones — not official Hyderabad ward boundaries
              </span>
            </div>
            ...
            <div className="map-alert-strip">
              ...
              {alerts.slice(0, 2).map((alert) => (
                <span
                  className={`mini-alert ${alert.severity === "CRITICAL" ? "critical" : ""}`}
                  key={alert.id}
                >
                  ▲ {alert.title} <em>demo</em>
                </span>
              ))}
          ...
          <aside className="right-rail">
            <section className="heat-panel forecast-panel">
              <h2>
                3-Day Heat Risk Forecast <small>synthetic demo</small>
              </h2>
              {forecast.map((item, index) => (
                <div
                  className={`forecast-card ${index === 1 ? "forecast-emphasis" : ""}`}
                  key={item.day}
                >
                  <div>
            ...
            <section className="heat-panel alerts-panel">
              <div className="section-title">
                <h2>
                  Active Alerts <small>synthetic demo</small>
                </h2>
              ...
              <div className="alerts-list">
                {alerts.map((alert) => (
                  <article
                    className={`warning-card alert-${alert.severity.toLowerCase()}`}
                    key={alert.id}
                  >
                    ...
                    <span>Demo zones: {alert.affectedZones.join(", ")}</span>
                    <p>{alert.message}</p>
                  </article>
                ))}
              </div>
        ...
        <section className="analytics-grid">
          <section className="heat-panel chart-panel">
            <h2>
              Heat Index Trend (Hyderabad) <small>synthetic demo</small>
            </h2>
            <div className="line-chart">
              <ResponsiveContainer width="100%" height={155}>
                <LineChart
                  data={analytics.heatIndexTrend}
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [`${value} °C`, "Heat index (demo)"]}
                  />
                  <Line
                    type="monotone"
                    dataKey="heatIndex"
          ...
          <section className="heat-panel distribution-panel">
            <h2>
              Risk Level Distribution <small>synthetic demo</small>
            </h2>
            <div className="distribution-content">
              <div className="distribution-donut">
                <PieChart width={160} height={160}>
                  <Pie
                    data={analytics.riskLevelDistribution}
                    dataKey="count"
                    nameKey="riskLevel"
                    cx="50%"
                    cy="50%"
              ...
              <div className="distribution-list">
                {analytics.riskLevelDistribution.map((item) => (
                  <span key={item.riskLevel}>
                    ...
                    />
                    {displayLevel(item.riskLevel)} <b>{item.count}</b>
                  </span>
                ))}
              </div>
            </div>
            ...
            {priorityWards.map((ward, index) => (
              ...
              >
                ...
                <div>
                  ...
                  <small>Risk Score: {ward.riskScore}</small>
                </div>
        ...
        <p className="data-disclaimer">
          ...
          this screen is clearly marked demo/placeholder content.
        </p>
      </main>
    </div>
  );
