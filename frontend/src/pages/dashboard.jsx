...
const chartColors = { VERY_HIGH: "#ff3b4e", HIGH: "#ff7200", MODERATE: "#ffbc08", LOW: "#18b878" };
...
function Dashboard() {
  ...
  return <div className="heatguard-shell">
    ...
    <main className="heatguard-main">
      ...
      <section className="analytics-grid"><section className="heat-panel chart-panel"><h2>Heat Index Trend (Hyderabad) <small>synthetic demo</small></h2><div className="line-chart"><ResponsiveContainer width="100%" height={155}><LineChart data={analytics.heatIndexTrend} margin={{ top: 25, right: 10, left: -25, bottom: 0 }}><CartesianGrid vertical={false} stroke="#253033" strokeDasharray="3 3" /><XAxis dataKey="label" tick={{ fill: "#9b9895", fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: "#9b9895", fontSize: 10 }} axisLine={false} tickLine={false} domain={["dataMin - 1", "dataMax + 1"]} /><Tooltip contentStyle={{ background: "#111517", border: "1px solid #584224", borderRadius: 4, color: "#f5eee7" }} labelStyle={{ color: "#ffbf08" }} formatter={(value) => [`${value} °C`, "Heat index (demo)"]} /><Line type="monotone" dataKey="heatIndex" stroke="#ffa3a0" strokeWidth={3} dot={{ fill: "#ffa3a0", r: 4 }} activeDot={{ r: 6 }}><LabelList dataKey="heatIndex" position="top" fill="#ffc3bd" fontSize={9} formatter={(value) => `${value}°`} /></Line></LineChart></ResponsiveContainer></div></section><section className="heat-panel distribution-panel"><h2>Risk Level Distribution <small>synthetic demo</small></h2><div className="distribution-content"><div className="distribution-donut"><PieChart width={160} height={160}><Pie data={analytics.riskLevelDistribution} dataKey="count" nameKey="riskLevel" cx="50%" cy="50%" innerRadius={53} outerRadius={75} paddingAngle={2}>{analytics.riskLevelDistribution.map((item) => <Cell fill={chartColors[item.riskLevel]} key={item.riskLevel} />)}</Pie></PieChart><span>WARDS</span></div><div className="distribution-list">{analytics.riskLevelDistribution.map((item) => <span key={item.riskLevel}><i className={`dot ${levelClass(item.riskLevel).replace("risk-", "")}`} />{displayLevel(item.riskLevel)} <b>{item.count}</b></span>)}</div></div></section><section className="heat-panel priority-panel"><h2>Top Priority Areas <small>demo</small></h2>{priorityWards.map((ward, index) => <div className="priority-row" key={ward.areaId}><b>{index + 1}</b><div><strong>{ward.name} – Demo area</strong><small>Risk Score: {ward.riskScore}</small></div><RiskBadge level={ward.riskLevel} /></div>)}</section></section>
